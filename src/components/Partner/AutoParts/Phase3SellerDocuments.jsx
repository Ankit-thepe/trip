...
import { InputField, FileUploadField, FormSection } from '../AutoParts/helpers'; // Reusing helpers

const Phase3SellerDocuments = ({ data, setData, errors }) => {
    const handleFileChange = (e) => setData({ ...data, [e.target.name]: e.target.files[0] });
    const handleChange = (e) => setData({ ...data, [e.target.name]: e.target.value });

    return (
        <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Business & Owner Documents</h2>
            <FormSection title="Business Documents">
                <InputField id="gstin" label="GSTIN" value={data.gstin} onChange={handleChange} error={errors.gstin} />
                <FileUploadField id="businessReg" label="Business Registration / UDYAM" fileName={data.businessReg?.name} onChange={handleFileChange} error={errors.businessReg} />
            </FormSection>
...
export default Phase3SellerDocuments;
